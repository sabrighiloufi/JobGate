...
module.exports = {
    create: async (req, res) => {
        ...
        const admin = new adminModel({...req.body, password, verificationCode: randomBytes(6).toString("hex")})
        const email = await adminModel.findOne({email: admin.email})
        if(email){
            ...
            admin.save(req.body, (err, item)=>{
                if(err){
                    ...
                    mailController(admin.email, "Welcome " + admin.fullname, 
    ...
    getAll: (req, res) => {
        adminModel.find({}, (err, items) => {
            if(err){
                ...
                res.status(201).json({message: "admins list", data: items})
            }
        })
    },
    getByID: (req, res) => {
        adminModel.findById(req.params.id, (err, item) => {
            if(err){
                ...
                res.status(201).json({message: "admin found", data: item})
            }
        })
    },
    ...
    ContactAdmin:(req, res)=>{
        adminModel.find({}, (err, items) => {
            if(err){
                res.status(406).json({message: "no admin to send email"})
            }else{
                ...
                for(const admin of items){
                    ...
                    transport.sendMail({
                        ...
                        <p style="box-sizing: border-box; font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, Helvetica, Arial, sans-serif, 'Apple Color Emoji', 'Segoe UI Emoji', 'Segoe UI Symbol'; position: relative; line-height: 1.5em; margin-top: 0; color: #b0adc5; font-size: 12px; text-align: center;">© 2022 JobGate. All rights reserved.</p>
